...
where the notation $\mathbb{E}_{s_0, a_0, ...,\sim\tilde{\pi}}$ indicates that actions are sampled $$\left.a_t\sim\tilde{\pi}(\cdot\right|s_t)$$. Let $\rho_\pi$ be the (unnormalized) discounted visitation frequencies  
