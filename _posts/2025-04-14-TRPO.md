...
Note that $L_\pi$ uses the visitation frequency $\rho_\pi$ rather than $\rho_{\tilde{\pi}}$, ignoring changes in state visitation density due to changes in the policy. However, if we have a parameterized policy $\pi_\theta$, where $\pi_\theta(a \mid s)$ is a differentiable function of the parameter vector $\theta$, then $L_\pi$ matches $\eta$ to first order. That is, for any parameter value $\theta_0$,    
