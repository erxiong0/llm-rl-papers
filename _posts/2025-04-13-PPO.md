...
During the rollout phase, agents interact with the environment using the old policy (\(\pi_{\text{old}}\)) to generate trajectories:
$$
\tau = \{s_t, a_t, r_t\}_{t=0}^{T-1}
$$

Subsequently, the Generalized Advantage Estimation (GAE) method computes advantage values \(\hat{A}_t\) through recursive temporal difference calculations:
$$
\begin{aligned}
...
\end{aligned}
$$
where \(\gamma\) is the discount factor and \(\lambda\) the GAE smoothing coefficient. Processed transitions \((s_t, a_t, \hat{A}_t, V_t^{\text{targ}})\) — with target values \(V_t^{\text{targ}} = \hat{A}_t + V_\theta(s_t)\) — are stored in an experience replay buffer (capacity: \(10^5 \sim 10^6\) transitions). This staged pipeline decouples data collection from policy updates while maintaining sample diversity through large-scale experience reuse.
...
2. **Policy Optimization**: Compute gradients for the clipped surrogate objective $\nabla_\theta L^{CLIP} = \mathbb{E}_t\left[\min\left(r_t(\theta)\hat{A}_t, {\rm clip}(r_t(\theta), 1-\epsilon, 1+\epsilon)\hat{A}_t\right)\nabla_\theta\log\pi_\theta(a_t|s_t)\right]$.  
  
3. **Value Network Update**: Minimize the value function's MSE loss through SGD: $\nabla_\theta L^{VF} = \mathbb{E}_{(s_t, V_t^{\rm targ})\sim {\rm buffer}}\left[(V_\theta(s_t)-V_t^{\rm targ}\nabla_\theta V_\theta(s_t))\right]$  
