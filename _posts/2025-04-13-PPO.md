...
Here, I aim to highlight the core computation codes, which will help you gain a deeper understanding of PPO in the RLHF (Reinforcement Learning from Human Feedback) scenario. For comprehensive implementation details, refer to the canonical guide:  The N Implementation Details of RLHF with PPO [[4]](#reference).  
...
3. Scale the logits by sampling temperature  
   ...
   ```
   It is said that without this scaling, the KL would rise faster than expected, and performanc would deteriorate [[4]](#reference).  

4. Pytorch Adam optimizer numerical issues w.r.t RLHF.
...
4. https://huggingface.co/blog/the_n_implementation_details_of_rlhf_with_ppo
