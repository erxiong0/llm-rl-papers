...
During the rollout phase, agents interact with the environment using the old policy ($\pi_{\rm old}$) to generate trajectories $\tau = \{s_t, a_t, r_t\}^{T-1}_{t=0}$. Subsequently, the Generalized Advantage Estimation (GAE) method computes advantage value $\hat{A}_t$ by through recursive temporal difference calculations:  
