...
The old policy \\( \pi_{\text{old}} \\) is used to generate trajectories:

\\[
\tau = \left\{ \left(s_t, a_t, r_t \right) \right\}_{t=0}^{T-1}
\\]

Subsequently, the Generalized Advantage Estimation (GAE) method computes the advantage value \\( \hat{A}_t \\) via recursive temporal difference calculations:
...
where \\( \gamma \\) is the discount factor and \\( \lambda \\) is the GAE smoothing coefficient.

Processed transitions \\( (s_t, a_t, \hat{A}_t, V_t^{\rm targ}) \\) with target values

\\[
V_t^{\rm targ} = \hat{A}_t + V_\theta(s_t)
\\]

are stored in an experience replay buffer (capacity: \\( 10^5 \sim 10^6 \\)).

This staged pipeline decouples data collection from policy updates while maintaining sample diversity through large-scale experience reuse.
